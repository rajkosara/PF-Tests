Given /^I navigate to (?:an|a|the) (.*) page$/ do |page_name|
  ...
  when 'create endeavour home page'
    @british_council.create_home_page_endeavour.load
  when 'create conqueror home page'
    @british_council.create_home_page_conqueror.load
  ...
  when 'regional language'
    @british_council.regional_language.load
  when 'regional settings'
    @british_council.regional_settings.load
  when 'create webform'
