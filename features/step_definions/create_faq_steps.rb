...
When /^I open the url path settings on the create faq page$/ do
  @british_council.create_faq.meta_config.url_path_setting.click
end

Then /^the generate automatic URL alias is checked by default on the create faq page$/ do
  @british_council.create_faq.meta_config.url_path_settings.generate_auto_url_checkbox.should be_checked
end

Then /^the filename field is disabled by default on the create faq page$/ do
