class Model::Domain::Course < Model::Domain::Content
  ...
  def initialize
    ...
    @tab_two_title = "Course Tab Two Title #{Time.current.strftime("%T")} #{String.random(4)}"
    @tab_two_body = "Course Tab Two Body #{Time.current.strftime("%T")} #{String.random(4)}"
    @tab_three_title = "Course Tab Three Title #{Time.current.strftime("%T")} #{String.random(4)}"
